class PharosRussiaMint {
    ...
    detectWallet() {
        ...
        // Проверяем OKX через window.okex (альтернативное название)
        if (window.okex && window.okex.request) {
            console.log('OKX Wallet (through okex) detected');
            return { provider: window.okex, name: 'OKX Wallet', icon: 'fas fa-wallet' };
        }
        ...
        if (window.ethereum && window.ethereum.request) {
            ...
            if (window.ethereum.isOkxWallet || window.ethereum.isOKXWallet) {
    ...
    showContractNotDeployedWarning() {
        // Показываем предупреждение что контракт не развернут
        document.getElementById('mintBtn').innerHTML = '<i class="fas fa-exclamation-triangle me-2"></i>Контракт не развернут';
        document.getElementById('mintBtn').disabled = true;
        document.getElementById('mintBtn').classList.add('btn-warning');
        document.getElementById('mintBtn').classList.remove('btn-primary');
        
        // Добавляем информационное сообщение
        const existingAlert = document.querySelector('.contract-not-deployed-alert');
        if (!existingAlert) {
            const infoDiv = document.createElement('div');
            infoDiv.className = 'alert alert-warning mt-3 contract-not-deployed-alert';
            infoDiv.innerHTML = `
                <h6><i class="fas fa-info-circle me-2"></i>Контракт не настроен</h6>
                <p class="mb-2">Для работы платформы нужно развернуть смарт-контракт:</p>
                <ol class="mb-2">
                    <li>Запустите: <code>npx hardhat run scripts/deploy.js --network pharos</code></li>
                    <li>Скопируйте адрес контракта</li>
                    <li>Установите переменную: <code>CONTRACT_ADDRESS=адрес_контракта</code></li>
                </ol>
                <small class="text-muted">После развертывания обновите страницу</small>
            `;
            
            // Вставляем после mint controls
            const mintControls = document.querySelector('.mint-controls');
            if (mintControls) {
                mintControls.appendChild(infoDiv);
            }
        }
    }
